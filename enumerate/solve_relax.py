...
def clone_stack(g, paths, groups, unsort, cache):
    ...
    for k, A, t in groups:
        ...
        out.append(np.einsum("nsd,cde->cnse", Y, A) + t[:, None, None, :])
    return np.concatenate(out, axis=0)[unsort]


class Clones:
    """a clone set, pre-digested into the form the inner loop wants."""

    def __init__(self, g, clones):
        self.list = clones
        self.groups, self.unsort = group_by_shift(g, clones)
        self.self_c = identity_index(clones)
        self.n = len(clones)
...
class Field:
    ...
    __slots__ = ("worst", "energy", "push", "near", "per_clone")

    def __init__(self, worst, energy, push, near, per_clone):
        self.worst, self.energy = worst, energy
        self.push, self.near, self.per_clone = push, near, per_clone


def encounters(g, paths, cl, d_min, cache, want_push=True, full=False):
    ...
    flanking samples -- the envelope theorem says this is the exact gradient of
    the penalty, because the instant of closest approach is itself a minimum.
    """
    n, S = paths.n, paths.S
    base = paths.extended() @ g.B                       # (n, S+1, 2)
    allp = clone_stack(g, paths, cl.groups, cl.unsort, cache)   # (C,n,S+1,2)
    diff = base[None, :, None, :, :] - allp[:, None, :, :, :]   # (C,i,j,S+1,2)
    d2 = (diff * diff).sum(axis=4)                      # (C,i,j,S+1) at samples
    if cl.self_c is not None:                           # a ball is not its own clone
        ii = np.arange(n)
        d2[cl.self_c, ii, ii, :] = np.inf
    ...
    # interval.  Everything else is far away and is never touched again.
    step = np.linalg.norm(base[:, 1:, :] - base[:, :-1, :], axis=2).max()
    lim = (d_min + 2.0 * step) ** 2
    ends = np.minimum(d2[..., :S], d2[..., 1:])
    hot = np.isfinite(ends) if full else (ends < lim)    # (C,i,j,S) bool
    ci, ii_, ji, si = np.nonzero(hot)

    A = diff[ci, ii_, ji, si, :]
    D = diff[ci, ii_, ji, si + 1, :] - A
    dd = (D * D).sum(axis=1)
    ...
    per_clone = seg.reshape(cl.n, -1).min(axis=1)
    worst = float(seg.min())
    near = seg.min(axis=(0, 2))                         # (n, S) per segment
    ...
    if want_push:
        ...
        pf = np.zeros((n, S + 1, 2))
        np.add.at(pf, (ii_, si), (1.0 - u)[:, None] * f)
        np.add.at(pf, (ii_, si + 1), u[:, None] * f)
        pc = pf[:, :S, :]
        pc[:, 0, :] += pf[:, S, :]                      # sample S is sample 0
        push = pc @ g.Binv
    return Field(worst, energy, push, near, per_clone)


def cull(g, paths, cl_all, d_min, slack, cache):
    """keep only the clones that ever come near a base path (fact 5).

    The test is symmetric under inversion -- the distance from ball i to the
    c-clone of ball j equals the distance from the c-inverse clone of i to
    ball j -- so the surviving set is still closed under inverses, which is
    what makes the push field equal and opposite on both partners."""
    f = encounters(g, paths, cl_all, d_min + slack, cache, want_push=False)
    keep = [c for c in range(cl_all.n) if f.per_clone[c] < d_min + slack]
    return Clones(g, [cl_all.list[c] for c in keep])
...
def solve(g, paths, clones_all, d_min, phases=14, rounds=90, gain0=1.0,
    ...
    cache = {}
    cap = 0.40 * d_min
    clones = cull(g, paths, clones_all, d_min, slack, cache)
    self_c = identity_index(clones)
    f = encounters(g, paths, clones, d_min, self_c, cache)
    gain = gain0
    history = []
    for phase in range(phases):
        ...
        for _ in range(rounds):
            if f.worst >= d_min:
                break
            V = paths.V
            trial = paths.trial(f.push, gain, cap)
            paths.set_V(trial)
            f2 = encounters(g, paths, clones, d_min, self_c, cache)
            if f2.energy <= f.energy:
                improved = (f.energy - f2.energy) / max(f.energy, TINY)
                f = f2
                gain = min(gain * 1.15, 6.0)
                stall = stall + 1 if improved < 2e-3 else 0
            else:
                paths.set_V(V)
                gain *= 0.5
                stall += 1
            ...
            if stall >= 12:
                break
        history.append(f.worst)
        if f.worst >= d_min:
            if verbose:
                print(f"  phase {phase}: cleared, worst {f.worst:.4f}")
            return f, True, phase
        ...
        if verbose:
            print(f"  phase {phase}: worst {f.worst:.4f} E {f.energy:.3e} "
        ...
        f = encounters(g, paths, clones, d_min, self_c, cache)
    f = encounters(g, paths, clones, d_min, self_c, cache)
    return f, f.worst >= d_min, phases


def prune(g, paths, clones_all, d_min, cache=None):
    """delete every kink that is not load bearing.

    A breakpoint stays only if straightening the path through it re-opens a
    collision -- so the kinks that survive are exactly the bounces.
    """
    cache = {} if cache is None else cache
    self_c = identity_index(clones_all)
    removed = 0
    for i in range(paths.n):
        k = 1
        while k < len(paths.B[i]):
            ...
            paths.drop(i, k)
            f = encounters(g, paths, clones_all, d_min, self_c, cache,
                           want_push=False)
            if f.worst >= d_min:
                removed += 1
...
def check_symmetry(g, paths, shift, span=4, radius_keep=1.0, frames=16):
    ...
    def orbit(t):
        pts, idx = [], []
        for Mc, vc, tau in wide:
            pts.append((paths.at(t - tau) @ Mc.T + vc) @ g.B)
            idx.append(np.arange(paths.n))
    ...
    for s in range(frames):
        ...
        for bi in range(paths.n):
            ...
            if not sel.any():
                continue
            same = p1[i1 == bi]
            d = np.linalg.norm(same[None, :, :] - turned[sel][:, None, :], axis=2)
...
def verify(g, paths, clones, d_min, radius):
    cache = {}
    f = encounters(g, paths, clones, d_min, identity_index(clones), cache,
                   want_push=False)
...
def run(gid="g226", starts=TINY_STARTS, drifts=TINY_DRIFTS, S=72, radius=0.075,
    ...
    prune(g, paths, clones, d_solve)
    worst, ratio = verify(g, paths, clones, d_min, radius)
    ok = worst >= d_min
