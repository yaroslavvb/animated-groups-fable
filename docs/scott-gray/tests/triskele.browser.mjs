...
// own bytes.
import {colourAt, fromPlane, frameAt, MAX_SCALE, uVolume, valuesAt} from '../triskele/renderer.mjs';
import {ELASTIC_GIVE, MAX_ZOOM_RATE, ZOOM_TAU} from '../triskele/momentum.mjs';
...
async function gesture(steps, at = [640, 400], {gap = 0} = {}) {
...
try {
  ...
  await page.goto(`${base}?taa=3&play=1&stats=1&scale=3000&shutter=0`); await ready();
  await page.waitForTimeout(400);
  ...
  // Zoomed in and playing, the shutter does switch itself on — and the gate is
  // on the SMEAR it would draw, travel × shutter, not on the travel. With the
  // shipped 0.3 shutter the 0.75 px of smear asks for 2.5 px of travel a frame,
  // which the pattern reaches only past about 2857 px per repeat; 2400 used to
  // engage it and no longer does. ?shutter=1 is the full box filter, and its
  // 0.75 px of travel 2400 passes easily.
  for (const [scale, on] of [[2400, false], [3000, true]]) {
    ...
    if (on) assert.match(text, /(81|144) taps/, `and its cost is counted in the taps: ${text}`);
  }
  ...
  assert.match(engaged, /shutter 3×1\.00 frame/, `the full box filter engages at a fast framing: ${engaged}`);
  ...
  await page.screenshot({path: `${shots}/triskele-${label}-elastic.png`});
