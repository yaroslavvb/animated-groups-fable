import {CENTER, createRenderer, createView, FIELD_BYTES, INITIAL_PHASE, LOOP_SECONDS, MAX_SCALE, MAX_TAA_LAYERS, MIN_SCALE, MOTION_PIXELS, RULES, scaleFor, SHUTTER, snapAngle, TAA_LAYERS, TEXTURE_SIZE, wrap} from './renderer.mjs';
...
// rule=u draws the shipped twill, the half-turn difference of the U channel;
// rule=v draws the V-channel alternate — the same two-colour group, the same
// strand width, and the same animation offset by 51/128 of a period rather than
// a second picture. Anything else falls back to u.
const rule = RULES.includes(params.get('rule')) ? params.get('rule') : 'u';
// Zooming out stops where one texel of the 96-texel reconstruction grid spans
// one device pixel.
const minScale = Math.max(MIN_SCALE, TEXTURE_SIZE / (devicePixelRatio || 1));
// scale=<CSS pixels per lattice length> fixes the initial scale; x=&y= place a
// lattice point at the centre (the lattice origin by default: the centre of the
// half turn that swaps black and white, where w vanishes at every phase);
// angle=<degrees> turns the pattern clockwise.
...
function renderStats() {
  ...
  nameLabel.setAttribute('aria-pressed', String(showStats));
  ...
  const shutterNote = renderer.maxLayers > 1
    ...
    : ' · shutter off';
  // Which channel the half-turn difference is taken of; only the alternate says so.
  const ruleNote = renderer.rule === 'u' ? '' : ` · rule ${renderer.rule.toUpperCase()}`;
  stats.textContent = `${playing ? `${Math.round(fps)} fps` : 'paused'} · ${canvas.width}×${canvas.height} px · quality ${renderer.quality.toFixed(2)} · ${Math.round(view.scale(width, height))} px per repeat${turned} · ${renderer.taps} taps${cadence}${shutterNote}${ruleNote}${gestureNote()}`;
}
function toggleStats() { showStats = !showStats; nameLabel.setAttribute('aria-pressed', String(showStats)); renderStats(); }
...
// about their midpoint, and a turn that ends within 4° of a quarter turn
...
function beginGesture(sawMulti = false) {
  ...
  // finger: the turn it made still has to settle onto a quarter when it ends.
...
for (const type of ['pointerup', 'pointercancel']) canvas.addEventListener(type, event => {
  ...
  // carries it on and eases onto the nearest quarter turn when it comes to rest — but
...
nameLabel.addEventListener('click', event => { toggleStats(); if (event.detail) nameLabel.blur(); });
...
function start() {
  try {
    renderer = createRenderer(canvas, field, {rule, view, pixelRatio, display: displayInterval(), taa, shutter, motion});
...
/** Reads a response body, reporting progress against `expected` bytes as it
 * goes. The pattern is the whole of the wait before the first frame, so the
 * notice counts it down rather than sitting still for several seconds on a slow
 * connection. A body with no length still streams; it just reports nothing. */
async function download(response, expected, report) {
  ...
  for (;;) {
    ...
    const percent = Math.min(99, Math.floor(100 * got / total));
...
/** The pattern, smallest transfer first.
 *
 * `field.f32z` is `field.f32` gzipped — 2.36 MB of float32 down to 0.68 MB, and
 * the whole of the time to first paint on a cold load. It is fetched under an
 * extension no server compresses or decompresses by itself, and the gzip magic
 * number decides whether it still needs inflating here: a host that chose to
 * decode it in transit hands over the plain field, and either way the bytes that
 * reach the decoder are the same. `DecompressionStream` is the only new platform
 * call, so anything without it (Safari before 16.4) takes the plain file, as does
 * anything that fails on the compressed one. */
async function loadField() {
  const report = percent => { notice.textContent = `Loading Weave Monochrome… ${percent} %`; };
  if (typeof DecompressionStream === 'function') {
    try {
      const packed = await fetch('./field.f32z');
      if (packed.ok) {
        const body = await download(packed, 680000, report);
        const gzipped = body[0] === 0x1f && body[1] === 0x8b;
        if (!gzipped) return body;
        const stream = new Blob([body]).stream().pipeThrough(new DecompressionStream('gzip'));
        const bytes = new Uint8Array(await new Response(stream).arrayBuffer());
        if (bytes.byteLength === FIELD_BYTES) return bytes;
      }
  ...
  return download(response, FIELD_BYTES, report);
}

try {
  const loaded = await loadField();
  const bytes = loaded.byteOffset === 0 && loaded.byteLength === loaded.buffer.byteLength ? loaded.buffer : loaded.slice().buffer;
