...
/** How long the turn takes to ease onto the nearest sixth when it snaps. */
export const SNAP_MS = 130;
/** A thrown turn aims at a sixth rather than at wherever the decay happens to
 * stop. When the glide starts, the angle it is heading for is worked out, and
 * the nearest sixth of a turn to *that* becomes the target — but only if the
 * correction is no bigger than half a sixth (the most it can ever be) **and** no
 * bigger than the throw itself, and only if that sixth lies the way the fingers
 * were turning. So a flick lands on a symmetry of the picture, exactly as one
 * released at rest does, while a small nudge is never dragged 30° it did not
 * ask for and nothing is ever pulled backwards. */
export const TURN_CATCH = Math.PI / 6;
 ...
 * by `ELASTIC_GIVE`. `snap` asks for the turn to ease onto the nearest sixth
...
export function createFling(velocity, {
  ...
  // Aiming at a sixth is a change of *rate*, not a correction bolted on at the
 ...
 * come to rest at `angle + rate × tau`, and this returns the nearest sixth of a
...
export function turnTargetFor(angle, rate, turnTau = TURN_TAU, catchWindow = TURN_CATCH) {
  ...
  const sixth = Math.round(landing / SIXTH_TURN) * SIXTH_TURN;
  const correction = sixth - landing;
  if (Math.abs(correction) > Math.min(catchWindow, Math.abs(thrown))) return null;
  // Never against the throw: a sixth behind where the fingers let go is not
  // somewhere a flick forwards may end up.
  if (Math.sign(sixth - angle) !== Math.sign(rate)) return null;
  return sixth;
}
...
function advanceTurn(state, dt) {
  ...
  state.turnRate = 0;
  // Coming to rest: onto the sixth the throw was aimed at, and failing that —
  // an unaimed glide, or one the catch declined — onto a sixth within 4° of
