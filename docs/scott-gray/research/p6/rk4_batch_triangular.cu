...
extern "C" __global__ void rk4_stage(
 ...
 const int diagonalUp=(y+n-1)%n*n+(x+n-1)%n, diagonalDown=(y+1)%n*n+(x+1)%n;
 const double a=du*(2.0/3.0)*di*(cur[o+l]+cur[o+r]+cur[o+up]+cur[o+down]+cur[o+diagonalUp]+cur[o+diagonalDown]-6*U)-rx+F*(1-U);
 const double c=dv*(2.0/3.0)*di*(cur[o+S+l]+cur[o+S+r]+cur[o+S+up]+cur[o+S+down]+cur[o+S+diagonalUp]+cur[o+S+diagonalDown]-6*V)+rx-(F+k)*V;
