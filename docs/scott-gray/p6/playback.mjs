...
export const screenToLattice=(p,tiles=2,options={})=>screenPointToLattice(p,viewTransform({family:'p6',tiles,...options}));
export const latticeToScreen=(p,tiles=2,options={})=>latticePointToScreen(p,viewTransform({family:'p6',tiles,...options}));
...
export function renderPixels(record,phase,{width=192,height=width,tiles=2,palette='ember',operation=null,viewMatrix=null,viewOrigin=null}={}){
  ...
  const view=viewTransform({family:'p6',tiles,viewMatrix,viewOrigin});
  for(let y=0;y<height;y++)for(let x=0;x<width;x++){
    const point=inverseOperation(screenPointToLattice([(x+.5)/width,(y+.5)/height],view),operation);
...
uniform sampler2D field;uniform sampler2D colors;uniform int size;uniform float phaseMix;uniform mat2 viewMatrix;uniform vec2 viewOrigin;uniform vec2 viewport;uniform vec2 limits;uniform int channel;
out vec4 outColor;
ivec2 wrap(ivec2 p){return (p%size+size)%size;}
vec4 at(ivec2 p){return texelFetch(field,wrap(p),0);}
void main(){
  vec2 lattice=viewOrigin+viewMatrix*(gl_FragCoord.xy/viewport-.5);
...
export function createPlayer(canvas,record,{onContextLost=()=>{}}={}){
  ...
  const uniforms=Object.fromEntries(['field','colors','size','phaseMix','viewMatrix','viewOrigin','viewport','limits','channel'].map(name=>[name,gl.getUniformLocation(program,name)]));
  ...
  return {backend:'WebGL playback',draw(phase,{tiles=2,palette='ember',viewMatrix=null,viewOrigin=null}={}){
    const view=viewTransform({family:'p6',tiles,viewMatrix,viewOrigin});
