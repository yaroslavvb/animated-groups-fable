...
// Zooming out stops where one node of the 36-node lattice spans one device pixel.
...
// lattice point at the centre (the turn centre p by default, so the picture's
// one fixed point sits under the middle of the screen); angle=<degrees> turns
...
// `?marks=` is accepted too: that is the spelling the colour catalog's own
// explorers use, and this page was lifted from one of their links.
const STORE_KEY = 'crosslet:generators';
const stored = (() => { try { return localStorage.getItem(STORE_KEY); } catch { return null; } })();
const askedGenerators = params.get('generators') ?? params.get('gen') ?? params.get('marks');
...
// Inside a host page's sandboxed frame — the sibling viewers are embedded that
// way elsewhere on the site — fullscreen is not permitted, so the button opens
// the viewer in its own tab instead.
