...
export function auditPhases({field,N,M,ops,trajectoryFrames,
  ...
  if(!Number.isInteger(N)||N<4||!Number.isInteger(M)||M<8||M%4)
    throw new Error('Phase audit needs N >= 4 and at least eight time frames, with M divisible by four.');
  ...
  const maps=ops.map((op,operation)=>{
    ...
    if(a*a+c*c!==1||b*b+d*d!==1||a*b+c*d!==0)throw new Error('Phase audit requires square-grid isometries.');
