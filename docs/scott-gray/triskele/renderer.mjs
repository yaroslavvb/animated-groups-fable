...
// something. MOTION_PIXELS is how wide the smear has to be before it is worth
// drawing — in CSS pixels, counting both the pattern's own boundaries and the
// view's motion — and a shutter of SHUTTER frame intervals smears a picture
// travelling `travel` pixels a frame over `travel × SHUTTER` of them. So the
// gate is on the smear, not on the travel: the picture must move
...
// reaches 2.5 px a frame at about 2857 CSS px per lattice length — against 857
...
// home framing moves 0.33 px a frame, a tenth of what the gate asks for.
...
export function createRenderer(canvas, planar, {tilePixels = scaleFor, center = CENTER, view = createView({tilePixels, center}), palette = PALETTE, pixelRatio = null, adaptive = true, display = null, taa = TAA_LAYERS, shutter = SHUTTER, motion = MOTION_PIXELS} = {}) {
  ...
  return {
    ...
    draw(phase, {continuous = false, moving = continuous, time = null} = {}) {
      ...
      // for blur. `travel > 0` is the first condition and not a consequence of
      // the last: with ?motion=0 — "integrate whenever anything moves at all" —
      // a picture standing still would otherwise satisfy `0 >= 0` and pay for
