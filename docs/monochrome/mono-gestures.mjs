// The endless camera's input for the Monochrome viewer.
//
// This is `../colour/colour-gestures.mjs`, which is itself
// `../scott-gray/gyre/app.mjs`'s camera packaged so a page can switch it off
// for the fixed "simulation width" framing. **One line differs**: the turn
// snap is an argument rather than the hexagonal sixth the Colour pages hard
// code, because twelve of the seventeen Monochrome families are on a square
// lattice and a square lattice settles onto a quarter of a turn. `momentum.mjs`
// is imported from the Colour tree rather than copied — the two pages have
// byte-identical needs there.
//
// One finger or a mouse pans; two fingers pan, zoom and turn about their
// midpoint, and a turn that ends within 4 degrees of an allowed turn snaps to
// it; a release keeps all three gliding. Every gesture — one finger, two
// fingers, the trackpad, a burst of wheel events — keeps the same list of
// samples and hands the same three-channel velocity to the same glide, so
// there is one piece of inertia in the viewer rather than one per input.
import {advanceFling, createFling, estimateVelocity, NO_THROW, trimSamples, WINDOW_MS} from '../colour/momentum.mjs?v=20260919-mono-v1';
import {snapAngle, LATTICES} from './mono-renderer.mjs?v=20260919-mono-v1';
...
export function createGestures(canvas, {view, size, onChange = () => {}, onSettle = () => {}, onTouch = () => {}, reducedMotion = {matches: false}, turnStep = view?.lattice?.turnStep ?? LATTICES.square.turnStep} = {}) {
  /** A quarter of a turn on the square lattice, a sixth on the triangular one. */
  const snap = angle => snapAngle(angle, turnStep);
