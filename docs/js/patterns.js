...
(() => {
  ...
  function overlay(fam, t, toS, scale, w, h) {
    ...
    fam.generators.forEach((gen) => {
      ...
      if (gen.kind === "rotation") {
        ...
        cycleChips(grp, t.cycles[gen.name], paletteOf(t.k), cx + rad + 8, cy - rad + 9);
        g.append(grp);
        return;
      }
      ...
      if (gen.kind === "glide") {
        ...
        const s = Math.sign(gl) || 1;
        const tx = sdx * s, ty = sdy * s;
      ...
      cycleChips(grp, t.cycles[gen.name], paletteOf(t.k), clx, cly + 11);
      g.append(grp);
    });
  ...
  function buildPane(t, g, fam) {
    ...
    cap.append(document.createTextNode("Generators of Γ — "),
      ...
      el("span", "cap-swap", "dashed = glide, half arrow = its direction and half-cell length"), document.createTextNode(", "),
      el("span", "cap-fix", "red = fixes the marked motif"),
      document.createTextNode("; colour chips give the colour cycle; the ring marks the seat. Translations are not drawn."));
    fig.append(cap);
